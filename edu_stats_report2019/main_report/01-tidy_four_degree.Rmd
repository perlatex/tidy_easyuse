...
### 学习深度与学业成绩
把深度学习得分均分成A、B、C、D四段，考察相应平均成绩
```{r, fig.height= 5.5, fig.width=5.5}
df_complete0 %>% 
  select(school, group, score_rate, test) %>% 
  mutate(
   score_rate = cut_interval(score_rate, 
                             n = 4#, 
                             #labels = c("A", "B", "C", "D")
                             ) 
  ) %>% 
  ...
  ggplot(aes(x = score_rate, y = mean_test)) +
  geom_point(size = 3) +
  geom_text(aes(label = round(mean_test, 2)), size = 3, vjust = -1) +
  labs(x = "学习深度得分率区间", 
       y = "平均考试成绩",
       title = "不同学习深度得分率区间下的学业成绩"
       ) 
```
