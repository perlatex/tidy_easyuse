...
df5_set %>% 
  filter(school == cur_schoolname) %>% 
  select(school, class_id, starts_with("num_effect")) %>%
...
df5_all %>% 
  filter(school == cur_schoolname | level == "district") %>% 
...
df5_set %>% 
  filter(school == cur_schoolname) %>% 
  select(class_id, starts_with("f_hard_")) %>%
  arrange(class_id == "全校") %>%
...
df5_set %>% 
  filter(school == cur_schoolname) %>% 
  select(class_id, starts_with("f_learning_")) %>% 
  arrange(is.na(class_id)) %>%
  rename_with(everything(), .fn = ~str_replace_all(.x, pattern = pairs56)) %>%
  flextable_print() %>% 
  fit_to_width(max_width = 6) %>%
...
`r ftext("空格", fp_text(color = "white"))`根据调查数据结果，针对数学课堂上的学习行为，五年级学生课堂参与的得分率为`r  tb$f_learning_activity_inclass`。`r fp`
...
index_activity_inclass <- df5_set %>% 
  filter(school == cur_schoolname) %>% 
  filter(level == "school") %>%
  select(f_learning_activity_inclass) %>% 
  pull(f_learning_activity_inclass)
