...
```{r, out.width='90%', fig.asp = 1, fig.align='center', fig.cap='调查框架和指标体系'}
...
`r ftext("空格", fp_text(color = "white"))`通过调查发现全区在以下方面表现良好：

- 与2019年的调查数据对比，除了睡眠时间，全区五年级在减轻学生课业客观负担上效果显著，家庭作业时间、音乐课开课情况、美术课开课情况、体育课开课情况、参加体育锻炼时间、统考次数、考试成绩公布和排名的达标率均有所提升。

- 草堂小学、鼓楼小学、实小明道分校、实小青华分校、实小文苑分校、实验小学、双眼井小学五年级在所有指标上的得分率都高于区平均得分率。

- 全区五年级学生感知的数学课业难度不高。

- 全区五年级学生对数学课堂、考试和作业的情绪以积极的体验为主。

- 全区五年级学生对数学的价值感、内在动机和效能感较高。

- 全区五年级学生的数学课堂参与度较高，课堂上学习过程中涉及到的较多的知识识记、深度理解和迁移运用。

- 全区五年级学生大多数都能够根据自己的数学学习情况进行自我评估、反思困惑，调整学习目标、方式。

`r ftext("空格", fp_text(color = "white"))`同时，数据也反映存在一定的问题需要改善：

- 五年级学生睡眠时间的达标率只有45.14%，与其他指标相比，亟待提高。

- 彩虹小学、金沙小学、少城小学、新华路小学五年级在所有指标上的得分率都低于区平均得分率。

- 全区五年级学生在数学课堂和考试中存在一定的消极情绪。

- 全区五年级学生感知的数学考试的难度明显高于课堂和作业。

- 全区五年级学生课堂上数学学习过程中涉及到的实践创新有增大空间。
...
df5_all %>%
  correlate_plot(vars = hard_class, title = "五年级")
```
...
```{r, fig.asp= 0.8, eval=FALSE}
df5_all %>%
  correlate_plot(vars = hard_homework, title = "五年级")
...
```{r, fig.asp= 0.8, eval=FALSE}
df5_all %>%
  correlate_plot(vars = hard_test, title = "五年级")
...
```{r, fig.asp= 0.8, eval=FALSE}
df5_all %>%
  correlate_plot(vars = f_feeling_class_negative, title = "五年级")
...
```{r, fig.asp= 0.8, eval=FALSE}
df5_all %>%
  correlate_plot(vars = f_feeling_class_positive, title = "五年级")
...
```{r, fig.asp= 0.8, eval=FALSE}
df5_all %>%
  correlate_plot(vars = f_feeling_test_positive, title = "五年级")
...
```{r, fig.asp= 0.8, eval=FALSE}
df5_all %>%
  correlate_plot(vars = f_feeling_homework_positive, title = "五年级")
...
```{r, fig.asp= 0.8, eval=FALSE}
df5_all %>%
  correlate_plot(vars = f_internal_driven, title = "五年级")

```

```{r, fig.asp= 0.8, eval=FALSE}
df5_all %>%
  correlate_plot(vars = f_external_driven, title = "五年级")
...
```{r, fig.asp= 0.8, eval=FALSE}
df5_all %>%
  correlate_plot(vars = f_learning_power, title = "五年级")
...
```{r, fig.asp= 0.8, eval=FALSE}
df5_all %>%
  correlate_plot(vars = f_activity_inclass, title = "五年级")
...
- 与2019年的调查数据对比，除了睡眠时间，全区六年级在减轻学生课业客观负担上效果显著，家庭作业时间、音乐课开课情况、美术课开课情况、体育课开课情况、参加体育锻炼时间、统考次数、考试成绩公布和排名的达标率均有所提升。

- 草堂小学、鼓楼小学、金沙小学、泡桐树小学、实小青华分校、实验小学六年级在所有指标上的得分率都高于区平均得分率。

- 全区六年级学生感知的语文课业难度不高。

- 全区六年级学生对语文课堂、考试和作业的情绪以积极的体验为主。

- 全区六年级学生对语文的价值感、内在动机较高。

- 全区六年级学生的语文课堂参与度较高，课堂上学习过程中涉及到的较多的知识识记、深度理解和迁移运用。

- 全区六年级学生大多数都能够根据自己的语文学习情况进行自我评估、反思困惑，调整学习目标、方式。

`r ftext("空格", fp_text(color = "white"))`同时，数据也反映存在一定的问题需要改善：

- 六年级学生睡眠时间的达标率只有39.43%，与其他指标相比，亟待提高。

- 彩虹小学、浣花小学、文翁实验小学、西财附小六年级在所有指标上的得分率都低于区平均得分率。

- 全区六年级学生在语文课堂和考试中存在一定的消极情绪。

- 全区六年级学生感知的语文考试的难度明显高于课堂和作业。

- 全区六年级学生对语文的效能感有提高空间。

- 全区六年级学生课堂上语文学习过程中涉及到的实践创新有增大空间。
...
`r ftext("空格", fp_text(color = "white"))`六年级各校学生感知的语文课业难度得分率统计如下：`r fp`
...
`r ftext("空格", fp_text(color = "white"))`
根据调查数据结果，全区六年级学生对课堂、考试和作业主要以积极的体验为主，具体表现为语文课上开心、语文考试高兴、语文作业享受；同时，也存在一定的消极情绪，如语文课上紧张、语文考试紧张、语文作业焦虑、语文作业厌倦等，在消极情绪中，课堂的消极情绪高于考试和作业。`r fp`





```{r, fig.width= 5, fig.height = 2.5, fig.cap= "全区六年级语文主观感受得分率"}
df6_all %>%
  filter(group == "district") %>%
  select(starts_with("f_feeling_")) %>%
  rename_with(everything(), .fn = ~str_replace_all(.x, pattern = pairs56)) %>%
...
`r ftext("空格", fp_text(color = "white"))`六年级各校学生主观感受到的语文课业情绪得分率统计如下：`r fp`
...
`r ftext("空格", fp_text(color = "white"))`六年级各校学生的语文学习态度价值得分率统计如下：`r fp`
...
`r ftext("空格", fp_text(color = "white"))`六年级语文课堂上涉及知识识记的得分率为85.64%，涉及深度理解的得分率为84.50%，涉及迁移运用的得分率为86.31%，涉及知识创新的得分率为80.30%。可见目前课堂上的学习已经在追求深度，除了奠定知识识记的基础，越来越关注知识的理解和运用，由数据可知最高层次的实践创新相对弱一些。`r fp`
...
`r ftext("空格", fp_text(color = "white"))`六年级各校学生语文学习与调节的得分率统计如下：`r fp`
...
df6_all_index %>% 
  ...
  scale_x_continuous(breaks = seq(0, 12, 1)) + 
  labs(x = "指标达标率高于全区均值的数量", y = "学生平均成绩",
       title = "指标达标率高于全区均值的数量与学生成绩的关联（六年级）"
       ) +
  theme(legend.position = "none") 
...
```{r, fig.asp= 0.8, eval=FALSE}
df6_all %>%
  correlate_plot(vars = hard_class, title = "六年级")
```
...
```{r, fig.asp= 0.8, eval=FALSE}
df6_all %>%
  correlate_plot(vars = hard_homework, title = "六年级")
```
...
df6_all %>%
  correlate_plot(vars = hard_test, title = "六年级")
```
...
```{r, fig.asp= 0.8, eval=FALSE}
df6_all %>%
  correlate_plot(vars = f_feeling_class_negative, title = "六年级")
```
...
```{r, fig.asp= 0.8, eval=FALSE}
df6_all %>%
  correlate_plot(vars = f_feeling_class_positive, title = "六年级")
```
...
```{r, fig.asp= 0.8, eval=FALSE}
df6_all %>%
  correlate_plot(vars = f_feeling_test_positive, title = "六年级")
```
...
```{r, fig.asp= 0.8, eval=FALSE}
df6_all %>%
  correlate_plot(vars = f_feeling_homework_positive, title = "六年级")
```
...
```{r, fig.asp= 0.8, eval=FALSE}
df6_all %>%
  correlate_plot(vars = f_internal_driven, title = "六年级")
```

```{r, fig.asp= 0.8, eval=FALSE}
df6_all %>%
  correlate_plot(vars = f_external_driven, title = "六年级")
```


```{r, fig.asp= 0.8, eval=FALSE}
df6_all %>%
  correlate_plot(vars = f_learning_power, title = "六年级")
```
...
```{r, fig.asp= 0.8, eval=FALSE}
df6_all %>%
  correlate_plot(vars = f_activity_inclass, title = "六年级")
```
...
```{r, fig.asp= 0.8, eval=FALSE}
df6_all %>%
  correlate_plot(vars = f_knowledge_creative, title = "六年级")
```
