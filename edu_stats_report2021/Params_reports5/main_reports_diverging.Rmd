...
`r ftext("空格", fp_text(color = "white"))`价值感、内在动机和自我效能感一直都是影响学生学业成绩的显著因素，全校五年级学生对数学的`r str_flatten(index_value, "、")`较高，说明五年级学生整体上认可`r ifelse(value_condition$f_learning_value_c,  "，数学学习的价值", "")``r ifelse(value_condition$f_internal_driven_c, "，有来自学好数学本身的动力", "")``r ifelse(value_condition$f_learning_power_c,  "，同时也认为自己能够学好", "")`。`r fp`
...
`r ftext("空格", fp_text(color = "white"))`全校五年级学生在课堂上学习过程中`r ifelse(length(index_knowledge_good) > 0, good_text, "")``r ifelse(length(index_knowledge_bad) > 0, bad_text, "")`。 `r fp`
