...
`r ftext("空格", fp_text(color = "white"))`根据调查数据结果，全校学生感知的语文考试、课堂和作业难度中，`r which.max(vec_hard) %>% names() `最大，为`r max(vec_hard) %>% scales::label_number(suffix = "%", accuracy = 0.01)()`，`r which.min(vec_hard) %>% names()`最小，为`r min(vec_hard) %>% scales::label_number(suffix = "%", accuracy = 0.01)()`。`r fp`




```{r, fig.width= 5, fig.height = 2, fig.cap= "六年级语文课业难度得分率"}
df6_set %>% 
  filter(school == cur_schoolname) %>% 
  filter(group == "school") %>%
...
`r ftext("空格", fp_text(color = "white"))`全校六年级学生感知的考试、课堂和作业难度中，考试难度最大，作业难度最小。`r fp`

- **语文主观感受**

`r ftext("空格", fp_text(color = "white"))`全校六年级学生对课堂、考试和作业的情绪以积极的体验为主。`r fp`
...
`r ftext("空格", fp_text(color = "white"))`全校六年级学生对语文的`r str_flatten(index_value, "、")`较高。`r fp`
...
`r ftext("空格", fp_text(color = "white"))`全校六年级学生的课堂参与度`r ifelse(index_activity_inclass >= 80, "较高", "有待提高")`。`r fp`
...
`r ftext("空格", fp_text(color = "white"))`全校六年级学生在课堂上学习过程中`r ifelse(length(index_knowledge_good) > 0, good_text, "")`
