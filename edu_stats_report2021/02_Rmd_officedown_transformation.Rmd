...
df5_all %>%
  ...
  geom_col(width = 0.6, fill = "#F6A173") +
...
`r ftext("空格", fp_text(color = "white"))`六年级学生报告语文老师、数学老师、英语老师、其他课的老师拖堂的比例分别为12.59%、16.66%、5.88%、3.52%，整体上75.29%的老师没有拖堂。可见，语文和数学老师拖堂的情况明显多于其他学科教师。具体如下：`r fp`
...
`r ftext("空格", fp_text(color = "white"))`五、六年级各校学生感知的语文/数学课业难度得分率统计如下：`r fp`

```{r tab.cap='五六年级学业难度得分率', tab.id='五六年级学业难度得分率'}
hard5 <- df5_all %>%
  select(school, starts_with("hard_"))
...
full_join(hard5, hard6, by = "school") %>%
  set_names(c("学校", 
              "五年级课堂难度",
              "五年级作业难度",
              "五年级考试难度",
...
```{r,eval=FALSE}
df5_scoring_rate  %>%
  mutate(group = score > mean(score)) %>%
  ggplot(aes(x = f_learning_value, y = score, color = group)) +
  geom_jitter() 
```
...
```{r,eval=FALSE}
df5_scoring_rate  %>%
  mutate(group = if_else(score >= mean(score), "A", "B") ) %>%
  ggplot(aes(x = group,  y = f_learning_value, color = group)) +
  geom_boxplot() +
  geom_jitter() +
  scale_x_discrete(labels = c("成绩高于均值的", "成绩低于均值的")) +
  labs(x = NULL, y = NULL) +
...
df5_scoring_rate %>%
  mutate(group = if_else(score >= mean(score), "A", "B") ) %>%
  ggplot(aes(x = group,  y = f_activity_inclass, color = group)) +
  geom_boxplot() +
  #geom_jitter() +
  scale_x_discrete(labels = c("成绩高于均值的", "成绩低于均值的")) +
  labs(x = NULL, y = NULL) +
...
```{r, fig.width= 7, fig.height = 6, fig.cap= "全区五年级数学不同学习成绩分组下各指标均值"}
library(colorspace) 
...
```{r tab.cap='六年级不同学习成绩分组下各指标均值', tab.id='六年级不同学习成绩分组下各指标均值', eval=FALSE}
df6_scoring_rate %>% 
  select(-starts_with("t")) %>%
  mutate(group = case_when(
    score >= quantile(score, probs = c(0.27, 0.73))[2] ~ "high", 
...
```{r, fig.width= 7, fig.height = 6, fig.cap= "全区六年级语文不同学习成绩分组下各指标均值"}
library(colorspace) 
...
df5_all %>%
  ...
  ggplot(aes(x = value, y = test_score)) +
  geom_point() +
...
df5_all_index %>% 
  ...
  labs(x = "指标达标率高于全区均值的数量", y = "学生平均成绩",
       title = "指标达标率高于全区均值的数量与学生成绩的关联（五年级）"
       ) +
  theme(legend.position = "none") 
